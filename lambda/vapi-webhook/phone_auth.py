"""
Phone-based authentication module for Voice/VAPI channel.

This module (Voice-specific):
...
4. Supports multiple concurrent callers (each phone has its own credentials)

Key difference from previous implementation:
- Old: Used Secrets Manager (single credential, overwritten by each caller)
- New: Uses DynamoDB (per-phone credentials, multiple concurrent callers)
"""
import boto3
import json
...
# DynamoDB table for voice credentials (per phone number)
# dev: us-east-1, prod: us-east-2
CREDENTIALS_TABLE = os.environ.get('VOICE_CREDENTIALS_TABLE', f'pf-syn-phone-credentials-{ENVIRONMENT}')

# Region configuration - voice Lambda and its DynamoDB are in the same region
...
def get_or_authenticate(from_phone: str, to_phone: str) -> Dict:
    ...
    Args:
        from_phone: Caller's phone number (VAPI customer.number)
        to_phone: System phone number (VAPI phoneNumber.number)

    Returns:
        Dict with: bearer_token, refresh_token, client_id, client_name, user_id, user_name, user_phone, timezone, exp
    ...
    logger.info(f"[PHONE_AUTH] Stored NEW credentials for user {credentials['user_id']} (phone: ***{from_clean[-4:]})")
...
def _get_existing_credentials(table, from_phone: str) -> Optional[Dict]:
    ...
    try:
        ...
        if 'Item' not in response:
            logger.info(f"[PHONE_AUTH] No credentials found for ***{from_phone[-4:]}")
            return None
        ...
        if remaining_seconds <= 0:
            logger.info(f"[PHONE_AUTH] Token EXPIRED for ***{from_phone[-4:]} - exp: {exp}, now: {now}")
            return None

        if remaining_seconds < TOKEN_REFRESH_BUFFER_SECONDS:
            logger.info(f"[PHONE_AUTH] Token expiring soon for ***{from_phone[-4:]} - {remaining_seconds:.0f}s ({remaining_mins:.1f}m) remaining, refreshing proactively")
            return None

        logger.info(f"[PHONE_AUTH] Token valid for ***{from_phone[-4:]} - {remaining_seconds:.0f}s ({remaining_mins:.1f}m) remaining")
        ...
        logger.warning(f"[PHONE_AUTH] Error reading credentials from DynamoDB: {e}")
        return None
...
def _store_credentials(table, phone_number: str, credentials: Dict):
    ...
    Each phone number gets its own row, so multiple callers can have
    ...
    try:
        ...
        logger.info(f"[PHONE_AUTH] Stored credentials for ***{phone_number[-4:]} (TTL: {ttl})")

    except Exception as e:
        logger.error(f"[PHONE_AUTH] Failed to store credentials in DynamoDB: {e}")
