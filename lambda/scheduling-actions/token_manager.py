...
AUTH_BASE_URLS = {
    "dev": "https://auth.dev.projectsforce.com",
    "staging": "https://auth.staging.projectsforce.com",
    "prod": "https://auth.projectsforce.com"
}
...
def get_auth_base_url():
    ...
    return AUTH_BASE_URLS.get(ENVIRONMENT, AUTH_BASE_URLS["dev"])
...
class TokenManager:
    ...
    def _get_credentials_from_secrets(self) -> Dict:
        ...
        Returns:
            Dictionary with email, password, and optionally refresh_token or bearer_token
        """
        try:
            response = self.secrets_client.get_secret_value(SecretId=self.secret_name)
            secret = json.loads(response['SecretString'])

            # If secret contains a bearer_token, we don't need email/password
            if 'bearer_token' in secret:
                logger.info("Secret contains bearer_token, will use static token")
                return secret

            # Otherwise, require email and password for authentication
            required_keys = ['email', 'encrypted_password']
            if not all(key in secret for key in required_keys):
                raise ValueError(f"Secret must contain either 'bearer_token' or {required_keys}")
