...
def _fallback_interpret(phrase: str, current_date: str) -> Dict[str, Any]:
    ...
    # Default: 10 days for better availability
    return {
        'start_date': tomorrow.strftime("%Y-%m-%d"),
        'end_date': (tomorrow + timedelta(days=9)).strftime("%Y-%m-%d"),
        'is_past': False,
        'date_type': 'day',
        'interpretation': f'Defaulting to next 10 days from {tomorrow.strftime("%Y-%m-%d")}',
        '_source': 'fallback'
    }
