            ...
            if 'projects' in data and isinstance(data['projects'], list) and len(data['projects']) > 0:
                ...
                if 'address' in first_project:
                    ...
                    if isinstance(addr, dict) and 'city' in addr:
                        city = addr['city']
                        logger.info(f"[OK] Extracted city from project address: {city}")
                        return city
    ...
    for msg in reversed(conversation_history):
        ...
        try:
            ...
            if 'project' in data and isinstance(data['project'], dict):
                ...
                if project_id:
                    logger.info(f"[OK] Resolved pronoun to project ID: {project_id} (from project details)")
                    return {
            ...
            if 'projects' in data and isinstance(data['projects'], list) and len(data['projects']) > 0:
                ...
                if project_id:
                    logger.info(f"[OK] Resolved pronoun to project ID: {project_id} (last project in list)")
                    return {
                        'project_id': str(project_id),
