...
def resolve_context_references(
    ...
    if 'project_id' not in entities and context.get('last_project'):
        ...
        if any(keyword in message.lower() for keyword in scheduling_keywords):
            ...
            logger.info(f"[RESOLVE] Implicit continuation: Using last_project={context['last_project']} (no explicit reference)")

    if references:
        logger.info(f"[RESOLVE] Resolved {len(references)} reference(s) in message")
...
def _extract_context_from_history(conversation_history: List[Dict]) -> Dict[str, Any]:
    ...
    for msg in reversed(conversation_history[-5:]):  # Last 5 messages
        ...
        if len(project_ids_found) > 1 and context['last_displayed_list'] is None:
            ...
            logger.debug(f"[LIST] Found list with {len(project_ids_found)} projects for positional refs")
    ...
    if context['last_displayed_list']:
        ...
        logger.debug(f"[OK] Using last displayed list ({len(context['project_ids'])} projects) for positional refs")
    else:
        context['project_ids'] = all_project_mentions
        logger.debug(f"[WARN]  No list found, using all mentions ({len(all_project_mentions)} projects)")
    ...
    if all_project_mentions:
        ...
        logger.debug(f"[LAST] last_project for pronouns: {context['last_project']}")
