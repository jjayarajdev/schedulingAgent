...
# Usage:
#   ./DEPLOY.sh
#   ./DEPLOY.sh --profile pf-aws
#   AWS_PROFILE=pf-aws ./DEPLOY.sh
##############################################################################
...
# Create pf-notes-dev table
echo ""
echo "Creating DynamoDB table: pf-notes-dev"

if aws_cmd dynamodb describe-table \
    --table-name "pf-notes-dev" \
    --region "$REGION" \
    &>/dev/null; then
    echo "  ℹ️  Table already exists: pf-notes-dev"
else
    echo "  → Creating table: pf-notes-dev"
    ...
    aws_cmd dynamodb create-table \
        ...
        --region "$REGION" \
        &>/dev/null && echo "  ✅ Table created: pf-notes-dev"
fi
...
EOFPOLICY
    ...
    if aws_cmd lambda get-function --function-name "$FUNCTION_NAME" --region "$REGION" &>/dev/null; then
        ...
        local max_wait=40  # Maximum 40 attempts (2 minutes with 3s sleep)
        local attempt=1
        while [ $attempt -le $max_wait ]; do
            ...
            STATE=$(aws_cmd lambda get-function \
                --function-name "$FUNCTION_NAME" \
                --region "$REGION" \
            ...
            if [ $((attempt % 5)) -eq 1 ] || [ "$STATUS" = "Successful" ] || [ "$STATUS" = "Failed" ]; then
                echo "    Attempt $attempt/$max_wait - Status: $STATUS, State: $STATE"
            fi
            ...
            if [ "$STATUS" = "Successful" ] && [ "$STATE" = "Active" ]; then
                ...
                REASON=$(aws_cmd lambda get-function \
                    --function-name "$FUNCTION_NAME" \
                    --region "$REGION" \
            ...
            if [ $attempt -eq $max_wait ]; then
                echo "  ⚠️  Timeout waiting for Lambda update (Status: $STATUS, State: $STATE)"
                echo "  → Continuing anyway - Lambda may be ready"
            fi
...
echo "  • DynamoDB-based session management (serverless)"
...
EOF
        ...
        # VPC execution policy not needed (DynamoDB is serverless, no VPC required)
        aws_cmd iam attach-role-policy \
            --role-name "$ORCHESTRATOR_ROLE_NAME" \
        ...
        cat > /tmp/orchestrator-policy.json <<EOF
{
  ...
  "Statement": [
    ...
    },
    {
      ...
      "Resource": [
        "arn:aws:lambda:${REGION}:*:function:pf-scheduling-actions",
        "arn:aws:lambda:${REGION}:*:function:pf-information-actions"
      ]
    },
    {
      "Effect": "Allow",
...
EOF
        ...
        echo "  ✅ IAM policy updated with Bedrock agent permissions, DynamoDB, inference-profile and Marketplace support"
    fi
    ...
    if aws_cmd lambda get-function --function-name "$ORCHESTRATOR_FUNCTION_NAME" --region "$REGION" &>/dev/null; then
        ...
        # First wait for any in-progress updates to complete
        echo "  → Checking for in-progress updates..."
        wait_count=0
        while [ $wait_count -lt 20 ]; do
            CURRENT_STATUS=$(aws_cmd lambda get-function-configuration \
                --function-name "$ORCHESTRATOR_FUNCTION_NAME" \
                --region "$REGION" \
            ...
            echo "    Waiting for in-progress update to complete... ($((wait_count + 1))/20)"
            sleep 5
        ...
        if ! aws_cmd lambda update-function-configuration \
            ...
            echo "  ⚠️  Configuration update skipped (may already be up to date or in progress)"
        fi
        ...
        while [ $attempt -le $max_wait ]; do
            ...
            if [ "$STATUS" = "Successful" ] && [ "$STATE" = "Active" ]; then
                ...
                REASON=$(aws_cmd lambda get-function-configuration \
                    --function-name "$ORCHESTRATOR_FUNCTION_NAME" \
                    --region "$REGION" \
            ...
            if [ $attempt -eq $max_wait ]; then
                echo "  ⚠️  Timeout waiting for configuration update (Status: $STATUS, State: $STATE)"
                echo "  → Continuing anyway - Lambda may be ready"
            fi
    ...
    echo "     - DYNAMODB_TABLE (required - will be set automatically)"
    echo "     - SUPERVISOR_AGENT_ID (required - will be set after agent creation)"
    echo "     - AWS_REGION (optional, defaults to us-east-1)"
    echo "     - USE_SUPERVISOR (optional, defaults to false for hybrid routing)"
    echo "     - ALLOW_DIRECT_LAMBDA (optional, defaults to true)"
    echo ""
    echo "  → DynamoDB table 'pf-sessions-dev' will be created in Step 1"
    echo "  → Environment variables will be configured after agent creation"
    echo ""
...
SCHEDULING_SCHEMA='{
  "functions": [
    ...
    {
      "name": "add_note",
      "description": "Add a note to a project. Notes are stored in DynamoDB and can be retrieved later.",
      "parameters": {
        "project_id": {"description": "Project ID to add the note to", "required": true, "type": "string"},
        "note_text": {"description": "The content of the note (max 2000 characters)", "required": true, "type": "string"},
        "author": {"description": "Who is creating the note (e.g., Customer, Agent, System)", "required": false, "type": "string"}
      }
    },
    {
      "name": "list_notes",
      "description": "List all notes for a specific project. Returns notes from DynamoDB sorted by timestamp (newest first).",
      "parameters": {
        "project_id": {"description": "Project ID to retrieve notes for", "required": true, "type": "string"}
      }
    }
  ]
}'
...
if aws_cmd lambda get-function --function-name pf-orchestrator --region "$REGION" &>/dev/null; then
    ...
    if aws_cmd dynamodb describe-table --table-name "$DYNAMODB_TABLE" --region "$REGION" &>/dev/null 2>&1; then
        echo "  ℹ️  DynamoDB table exists: $DYNAMODB_TABLE"
    else
        echo "  ⚠️  DynamoDB table not found: $DYNAMODB_TABLE"
        echo "  → Table should have been created in Step 1"
    fi
...
EOF
        ...
        if aws_cmd lambda update-function-configuration \
            ...
            echo "  ✅ DynamoDB table configured: $DYNAMODB_TABLE"
        else
...
echo "  ────────────────────────────────────────────────────"
echo ""
echo "  STEP 3: Deploy API Gateway (OPTIONAL)"
echo "  ────────────────────────────────────────────────────"
echo "  DynamoDB session storage is already configured (serverless)"
echo "  To add API Gateway for HTTP endpoint access:"
echo ""
