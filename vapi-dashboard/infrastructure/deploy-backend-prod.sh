#!/bin/bash
# ============================================================================
# PF-SYN VAPI Dashboard - Deploy Backend to PROD
# Region: us-east-2
# ============================================================================
...
echo "============================================"
echo "Deploying VAPI Dashboard Backend - PROD"
echo "============================================"
...
if AWS_PROFILE=$AWS_PROFILE aws lambda get-function --function-name $AUTH_LAMBDA --region $REGION 2>/dev/null; then
  ...
  AWS_PROFILE=$AWS_PROFILE aws lambda create-function \
    ...
    --timeout 30 \
    --memory-size 256 \
    --environment "Variables={ENVIRONMENT=${ENV},USERS_TABLE=${USERS_TABLE},TENANTS_TABLE=${TENANTS_TABLE},JWT_SECRET=CHANGE-THIS-TO-SECURE-SECRET-PROD}" \
    --region $REGION \
    --query '[FunctionName, LastModified]' \
    --output table
fi
...
if [ -z "$VAPI_API_KEY" ]; then
  echo "  ERROR: VAPI_API_KEY required for production!"
  echo "  Set it with: export VAPI_API_KEY=your-key && ./deploy-backend-prod.sh"
  exit 1
fi
...
if AWS_PROFILE=$AWS_PROFILE aws lambda get-function --function-name $API_LAMBDA --region $REGION 2>/dev/null; then
  ...
  AWS_PROFILE=$AWS_PROFILE aws lambda create-function \
    ...
    --timeout 60 \
    --memory-size 512 \
    --environment "Variables={ENVIRONMENT=${ENV},TENANTS_TABLE=${TENANTS_TABLE},VAPI_API_KEY=${VAPI_API_KEY},JWT_SECRET=CHANGE-THIS-TO-SECURE-SECRET-PROD}" \
    --region $REGION \
    --query '[FunctionName, LastModified]' \
    --output table
fi
...
if [ -z "$API_ID" ]; then
  echo "  ERROR: API Gateway not found. Run setup-prod.sh first."
  exit 1
fi
...
echo ""
echo "============================================"
echo "PROD Backend Deployment Complete!"
echo "============================================"
echo ""
echo "API Endpoints:"
echo "  Base URL: https://${API_ID}.execute-api.${REGION}.amazonaws.com"
echo ""
echo "IMPORTANT: Update JWT_SECRET in Lambda environment variables!"
echo ""
